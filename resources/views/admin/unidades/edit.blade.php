...
@section('content')
    ...
    {!! Form::model($unidade, ['method' => 'PUT', 'route' => ['admin.unidades.update', $unidade->id], 'files' => true,]) !!}
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    @if ($unidade->imagen_unidad)
                        <a href="{{ asset(env('UPLOAD_PATH').'/'.$unidade->imagen_unidad) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/'.$unidade->imagen_unidad) }}"></a>
                    @endif
                ...
                <div class="col-xs-12 form-group">
                    ...
                    <div class="photo-block">
                        ...
                        <div class="files-list">
                            @foreach($unidade->getMedia('recursos') as $media)
                                <p class="form-group">
                                    <a href="{{ $media->getUrl() }}" target="_blank">{{ $media->name }} ({{ $media->size }} KB)</a>
                                    <a href="#" class="btn btn-xs btn-danger remove-file">Remove</a>
                                    <input type="hidden" name="recursos_id[]" value="{{ $media->id }}">
                                </p>
                            @endforeach
                        </div>
                    </div>
